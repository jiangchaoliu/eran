...
for i in range(len(x_test)):
    x_test_inc = x_test[i:i+1];
    y_test_inc = y_test[i:i+1];


    preds = np.argmax(classifier.predict(x_test_inc), axis=1)
    acc = np.sum(preds == np.argmax(y_test_inc, axis=1)) / y_test_inc.shape[0]
    print("\nTest accuracy: %.2f%%" % (acc * 100))
    # Craft adversarial samples with FGSM

    epsilon = .1  # Maximum perturbation
    adv_crafter = HopSkipJump(classifier)
    x_test_adv = adv_crafter.generate(x=x_test_inc)
    ...
    if y != pred:
        continue
    x_adv = x_test_adv[0].reshape(1,32,32,3)
